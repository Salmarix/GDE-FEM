function [ B,C,B_petrov,C_petrov ] = Coagulation_MC_quadrature_matrix_creator( nodes,kernel_function,gaussian_points,epsilon)
%COAGULATION_QUADRATURE_MATRIX_CREATOR calculates coagulation FEM matrixes 
% by using the three or five point gaussian quadrature. Gaussian quadrature 
% is preferable as it it numerically light and accurate method for the 
% matrix calculation
%
% Teemu Salminen
...
for jj = 1:length(nodes)-1
    ...
    for kk = 1:min(e_ind(2),length(nodes)-1)
        ...
        % Finding the test functions which produces particles corresbonding the
        % test function element phi_j. 
        ...
        if ~isempty(row)
            ...
            for ii = 1:length(row)
                ...
                % Different possible integrations depending of the result
...
%                         i_interval = [H_support(row(ii),1)+x1,H_support(row(ii),3)-x2];
                    ...
                    elseif interval_check == [0,0]
                    
                        v_interval = [H_support(row(ii),1)+element_boundary(kk,1),H_support(row(ii),3)+int_interval(2)];
                    ...
                    %
                    B_loc1(kk,row(ii)) = B_loc1(kk,row(ii))+h_j/2*sum(w_j.*inner_integral1.*phi_j1);
                    %                 B_loc1(kk,row(ii)+1) = B_loc1(kk,row(ii)+1)+h_j*sum(w_j.*inner_integral12.*phi_j1);
                    B_loc1(kk+1,row(ii)) = B_loc1(kk+1,row(ii))+h_j/2*sum(w_j.*inner_integral2.*phi_j1);
                    %                 B_loc1(kk+1,row(ii)+1) = B_loc1(kk+1,row(ii)+1)+h_j*sum(w_j.*inner_integral22.*phi_j1);
                    B_loc2(kk,row(ii)) = B_loc2(kk,row(ii))+h_j/2*sum(w_j.*inner_integral1.*phi_j2);
                    %                 B_loc2(kk,row(ii)+1) = B_loc2(kk,row(ii)+1)+h_j*sum(w_j.*inner_integral12.*phi_j2);
                    B_loc2(kk+1,row(ii)) = B_loc2(kk+1,row(ii))+h_j/2*sum(w_j.*inner_integral2.*phi_j2);


                    if nargin > 3

                        B_PGloc1(kk,row(ii)) = B_PGloc1(kk,row(ii))+h_j/2*sum(w_j.*inner_integral1.*phiPG_j1);
                        B_PGloc1(kk+1,row(ii)) = B_PGloc1(kk+1,row(ii))+h_j/2*sum(w_j.*inner_integral2.*phiPG_j1);
                        B_PGloc2(kk,row(ii)) = B_PGloc2(kk,row(ii))+h_j/2*sum(w_j.*inner_integral1.*phiPG_j2);
                        B_PGloc2(kk+1,row(ii)) = B_PGloc2(kk+1,row(ii))+h_j/2*sum(w_j.*inner_integral2.*phiPG_j2);
    ...
    for kk = 1:length(nodes)-1
        ...
        C_loc1(jj,kk) = C_loc1(jj,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j1.*phi_j1);
        C_loc1(jj+1,kk) = C_loc1(jj+1,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j2.*phi_j1);
        C_loc1(jj,kk+1) = C_loc1(jj,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j1.*phi_j1);
        C_loc1(jj+1,kk+1) = C_loc1(jj+1,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j2.*phi_j1);


        C_loc2(jj,kk) = C_loc2(jj,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j1.*phi_j2);
        C_loc2(jj+1,kk) = C_loc2(jj+1,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j2.*phi_j2);
        C_loc2(jj,kk+1) = C_loc2(jj,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j1.*phi_j2);
        C_loc2(jj+1,kk+1) = C_loc2(jj+1,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j2.*phi_j2);

        if nargin > 3

            C_PGloc1(jj,kk) = C_PGloc1(jj,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j1.*phiPG_j1);
            C_PGloc1(jj+1,kk) = C_PGloc1(jj+1,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j2.*phiPG_j1);
            C_PGloc1(jj,kk+1) = C_PGloc1(jj,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j1.*phiPG_j1);
            C_PGloc1(jj+1,kk+1) = C_PGloc1(jj+1,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j2.*phiPG_j1);


            C_PGloc2(jj,kk) = C_PGloc2(jj,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j1.*phiPG_j2);
            C_PGloc2(jj+1,kk) = C_PGloc2(jj+1,kk)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral1.*phi_j2.*phiPG_j2);
            C_PGloc2(jj,kk+1) = C_PGloc2(jj,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j1.*phiPG_j2);
            C_PGloc2(jj+1,kk+1) = C_PGloc2(jj+1,kk+1)+h_j/2*sum(w_j.*v_kk.^(-1).*inner_integral2.*phi_j2.*phiPG_j2);
...
% Gaussian quadrature function for the inner integrals for formation term.
    function int_value = quadrature(integration_limits,v_j_value,phi_k,basis_i)
        ...
        int_value = (h/2)*sum(w.*(v_j_value-v_k).^(-1).*kernel_function(v_j_value-v_k,v_k).*phi...
