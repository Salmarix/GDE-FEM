function [N_diff,n_diff,d_middle,d_edges,tt] = discrete_GDE_solver_final(d_min,d_max,coag_kernel,...
    n_init,source,removal,time_step,t_max,solver,dN0,n0,phenom)
% Teemu Salminen
% University of Eastern Finland
% Department of Technical Physics
% 2020
%
% This codes calculates the temporal evolution of discrete GDE. This is the
% most current version of code as also possible sources and removal
...
%            (N_i = n_i*Dd_i)
%   source : Source term at the lower boundary (or somewhere else)
%   removal : Removal process of particles
...
if strcmp('Euler',solver) | strcmp('euler',solver)
    ...
    for ii = 1:length(tt)-1
        ...
        if strcmp(phenom,'GDE')
            if ~isempty(source) && ~isempty(removal)
                % Source term
                S = [source(tt(ii)),zeros(1,length(d_middle)-1)]';
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(0.5*F.*apu1-R.*apu2+C-Rem)*N_diff(:,ii)+time_step*S;
            ...
            elseif ~isempty(source)
                ...
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
            elseif ~isempty(removal)
                % Source term
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(0.5*F.*apu1-R.*apu2+C-Rem)*N_diff(:,ii);
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
        ...
        elseif strcmp(phenom,'coag')
            if ~isempty(source) && ~isempty(removal)
                ...
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(0.5*F.*apu1-R.*apu2-Rem)*N_diff(:,ii)+time_step*S;
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
            ...
            elseif ~isempty(removal)
                ...
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(0.5*F.*apu1-R.*apu2-Rem)*N_diff(:,ii);
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
        ...
        elseif strcmp(phenom,'cond')
            if ~isempty(source) && ~isempty(removal)
                ...
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(C-Rem)*N_diff(:,ii)+time_step*S;
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
            ...
            elseif ~isempty(removal)
                ...
                Rem = removal(v);
                Rem = Rem(:);
                N_diff(:,ii+1) = N_diff(:,ii)+time_step*(C-Rem)*N_diff(:,ii);
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
...
%         So = [source(tt(ii)),zeros(1,length(d_middle)-1)]';
        ...
        if strcmp(phenom,'GDE')
            if ~isempty(source) && ~isempty(removal)
                % Source term
                So = [source(tt(ii)),zeros(1,length(d_middle)-1)]';
                Rem = removal(d_middle);
                Rem = diag(Rem(:));
                S = eye(length(d_middle))+(time_step/2)*(0.5*F.*apu1-R.*apu2+C-Rem);
                S2 = eye(length(d_middle))-(time_step/2)*(0.5*F.*apu1-R.*apu2+C-Rem);
...
%                 So = [source(tt(ii)),zeros(1,length(d_middle)-1)]';
        ...
        elseif strcmp(phenom,'coag')
            if ~isempty(source) && ~isempty(removal)
                ...
                Rem = removal(d_middle);
                Rem = Rem(:);
                S = eye(length(d_middle))+(time_step/2)*(0.5*F.*apu1-R.*apu2-Rem);
                S2 = eye(length(d_middle))-(time_step/2)*(0.5*F.*apu1-R.*apu2-Rem);
                N_diff(:,ii+1) = (S2\S)*N_diff(:,ii)+S2\(time_step*So);
                N_diff(N_diff < 0) = 0;
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
...
%                 So = [source(tt(ii)),zeros(1,length(d_middle)-1)]';
        ...
        elseif strcmp(phenom,'cond')
                ...
                Rem = removal(d_middle);
                Rem = Rem(:);
                S = eye(length(d_middle))+(time_step/2)*(C-Rem);
                S2 = eye(length(d_middle))-(time_step/2)*(C-Rem);
                N_diff(:,ii+1) = (S2\S)*N_diff(:,ii)+S2\(time_step*So);
                n_diff(:,ii+1) = N_diff(:,ii+1)/d_width;
