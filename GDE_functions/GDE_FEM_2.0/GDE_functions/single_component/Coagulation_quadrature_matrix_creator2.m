function [ B,C,B_petrov,C_petrov ] = Coagulation_quadrature_matrix_creator2( nodes,kernel_function,gaussian_points,epsilon)
%%COAGULATION_QUADRATURE_MATRIX_CREATOR2 calculates coagulation FEM matrixes
%by using the three or five point gaussian quadrature.
%
% Updated version from the Coagulation_quadrature_matrix_creator. This code
% calculates the integrals for the coagulation FE matrices more accurately
% than the original one! IT IS PREFERABLE TO USE THIS CODE
%
% Updated by
% Teemu Salminen
...
%                         i_interval = [H_support(row(ii),1)+x1,H_support(row(ii),3)-x2];
    ...
    B{jj,1} = sparse(B{jj,1}+0.5*B_loc1);
    B{jj+1,1} = sparse(B{jj+1}+0.5*B_loc2);

    if nargin > 3
        B_petrov{jj,1} = sparse(B_petrov{jj,1}+0.5*B_PGloc1);
        B_petrov{jj+1,1} = sparse(B_petrov{jj+1}+0.5*B_PGloc2);
    end
