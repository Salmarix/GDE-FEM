...
for jj = 1:length(tt)-1
    ...
    if nargin > 9
        S2= (M\(-M_b*dC_in(tt(jj+1))+K_b*C_in(jj+1)))+source(jj+1);
        S1= (M\(-M_b*dC_in(tt(jj))+K_b*C_in(jj)))+source(jj);
        
        % Without source term
    elseif ~isempty(C_in) && nargin <= 9
        S2= (M\(-M_b*dC_in(tt(jj+1))+K_b*C_in(jj+1)));
        S1= (M\(-M_b*dC_in(tt(jj))+K_b*C_in(jj)));
